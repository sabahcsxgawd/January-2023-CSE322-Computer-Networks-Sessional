...
NS_LOG_COMPONENT_DEFINE("WiFi_High_Rate_Static");

int total_packets_sent = 0;
int total_packets_received = 0;
int total_received_bytes = 0;

void receiver_callback(Ptr<const Packet> packet, const Address &address) {
    total_packets_received++;
    total_received_bytes += packet->GetSize();
}

void sender_callback(Ptr<const Packet> packet) {
    total_packets_sent++;
}
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("nodes", "Number of static wireless nodes", nodes);
    cmd.AddValue("flows", "Number of flows", flows);
    cmd.AddValue("pps", "How many packets per second", packets_per_sec);
    cmd.AddValue("cov", "Coverage Area = Tx_range * coverage", coverage);
    cmd.AddValue("file", "Data Output File", outputFile);

    cmd.Parse(argc, argv);

    outputFile = "scratch/static/" + outputFile + ".dat";
    ...
    channel.AddPropagationLoss("ns3::RangePropagationLossModel", "MaxRange", ns3::DoubleValue(coverage * Tx_range));
    ...
    ap_positions->Add(Vector(0.0, 0.0, 0.0));
    ap_positions->Add(Vector(ap_distance, 0.0, 0.0));
    ...
    mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator", 
                                        ...
                                        "Rho", ns3::PointerValue(diskRho));

    mobility.Install(wifiStaNodes_senders);

    mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator", 
                                        "X", ns3::DoubleValue(ap_distance),
                                        "Y", ns3::DoubleValue(0.0),
                                        "Rho", ns3::PointerValue(diskRho));
